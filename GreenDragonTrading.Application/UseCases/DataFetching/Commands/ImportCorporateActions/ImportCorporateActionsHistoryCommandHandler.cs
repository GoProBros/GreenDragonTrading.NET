...
public class ImportCorporateActionsHistoryCommandHandler(
    IUnitOfWork uow,
    IDnseService dnseService,
    ILogger<ImportCorporateActionsHistoryCommandHandler> logger)
    : IRequestHandler<ImportCorporateActionsHistoryCommand, ApiResponse<ImportCorporateActionsResult>>
{
    private readonly IUnitOfWork _uow = uow;
    private readonly IDnseService _dnseService = dnseService;
    private readonly ILogger<ImportCorporateActionsHistoryCommandHandler> _logger = logger;

    public async Task<ApiResponse<ImportCorporateActionsResult>> Handle(
        ImportCorporateActionsHistoryCommand request,
        CancellationToken cancellationToken)
    {
        try
        {
            var tickers = (await _uow.Symbols.GetAllTickersAsync(cancellationToken))
            ...
            if (tickers.Count == 0)
            {
                return ApiResponse<ImportCorporateActionsResult>.Failure("No symbols available to import corporate actions.");
            }
            ...
            foreach (var ticker in tickers)
            {
                ...
                while (true)
                {
                    var response = await _dnseService.GetCorporateActionsHistoryAsync(
                        ticker,
                        page,
                        cancellationToken);
            ...
            var result = new ImportCorporateActionsResult(
                ...
                updatedCount,
                skippedCount,
                $"Corporate actions history import completed. Inserted {insertedCount}, updated {updatedCount}.");

            return ApiResponse<ImportCorporateActionsResult>.Success(result, result.Message);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error importing corporate actions history from DNSE");
            return ApiResponse<ImportCorporateActionsResult>.Failure("Failed to import corporate actions history from DNSE.");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var ticker = ResolveTicker(item, fallbackTicker);
            ...
            if (validTickers != null && !validTickers.Contains(ticker))
            {
                skipped++;
                continue;
            }
    ...
    private static string ResolveTicker(DnseCorporateActionItem item, string fallbackTicker)
    {
